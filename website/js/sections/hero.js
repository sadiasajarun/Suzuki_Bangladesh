...
(function heroInit() {
  ...
  /* ============================================================
     BIKE DATA
     ============================================================
     Sizing reference: GSX-R150 is the standard (scale 1.0). Other bikes
     have their `scale` tuned so all four read at the same on-screen size
     after the trim/knockout passes. Scooters (access-125) need more
     scale because their wider/shorter aspect ratio leaves vertical
     empty space inside the 4:3 wrap, making them look smaller by
     default. `tolerance` controls the white-knockout threshold per bike
  ...
  const BIKES = [
    {
      ...
      anchors: { engine: [0.516, 0.587], power: [0.527, 0.483], mileage: [0.542, 0.333] },
    },
    {
      id: 'gsx-r150',
      ...
      cc: 147,
      knockout: false,
      scale: 1.0,                                  /* REFERENCE — all bikes target this size */
      specs: { engine: 147, power: 19.0, mileage: 45 },
      anchors: { engine: [0.499, 0.564], power: [0.528, 0.441], mileage: [0.516, 0.324] },
    },
    {
      id: 'gixxer-150',
      ...
      anchors: { engine: [0.519, 0.589], power: [0.532, 0.488], mileage: [0.549, 0.366] },
    },
    {
      id: 'access-125',
      ...
      name: ['Access', ' ', '125'],
      price: '1,89,950',
      img: 'assets/images/bikes/access-125.webp',
      cc: 124,
      knockout: true,
      tolerance: 60,                               /* aggressive knockout for white-bg JPEG */
      scale: 1.0,                                  /* fallback only — auto-scale overrides at runtime */
      specs: { engine: 124, power: 8.7, mileage: 64 },
      anchors: { engine: [0.384, 0.627], power: [0.295, 0.553], mileage: [0.478, 0.479] },
    },
  ];
  ...
  /* ------------------------------------------------------------
     ANCHOR-PICKER DEBUG MODE  (URL flag: ?anchors)
     ------------------------------------------------------------
     When the page is opened with ?anchors in the URL, we suppress
     the carousel auto-advance and expose hero internals on
     `window.__heroDebug` so the debug helper at
     js/sections/hero-anchor-debug.js can run.

     This block is a no-op in production (URL flag absent).
     ------------------------------------------------------------ */
  const ANCHOR_DEBUG = (() => {
  ...
  if (ANCHOR_DEBUG) {
    // Pin auto-advance off for the whole session
    userInteracted = true;
    document.body.classList.add('is-anchor-debug');
  }
  ...
  /* ------------------------------------------------------------
     knockoutWhite / trimImage now resolve with
     `{ url, width, height }` so preprocessKnockouts() can
     auto-compute --bike-scale. Both still fall back to a plain
     url string if anything throws (downstream code reads .url
     defensively).
     ------------------------------------------------------------ */
  ...
  /* ------------------------------------------------------------
     preprocessKnockouts — runs on page load.

     1. For every bike: trim transparent borders (and knock out
        white if `knockout: true`). Capture the trimmed dimensions
        on the bike object (`b.trimmedWidth/Height`).

     2. After all bikes processed, auto-compute `b.scale` so each
        bike's trimmed height matches the reference (GSX-R150)
        when rendered. Formula:

             b.scale = referenceHeight / b.trimmedHeight

        This makes the system size-agnostic — drop in a 4000×3000
        photo or a 600×450 thumbnail and the hero renders it at
        exactly the same on-screen height as the reference. The
        manual `scale` field on each bike now serves as a fallback
        only (used if dimension reads fail).
     ------------------------------------------------------------ */
  async function preprocessKnockouts() {
    await Promise.all(BIKES.map(async (b) => {
      const tol = b.tolerance ?? 28;
      try {
        const result = b.knockout ? await knockoutWhite(b.img, tol) : await trimImage(b.img);
        b.img = result.url;
        b.trimmedWidth = result.width;
        b.trimmedHeight = result.height;
      } catch (err) {
        console.warn('[hero] preprocess failed for', b.id, err);
      }
    }));

    // Auto-scale pass — find the reference (GSX-R150) and compute
    // a scale factor for every other bike so they all render at
    // the same on-screen height regardless of source dimensions.
    try {
      const ref = BIKES.find((b) => b.id === 'gsx-r150') || BIKES[0];
      if (ref && ref.trimmedHeight > 0) {
        BIKES.forEach((b) => {
          if (b.trimmedHeight > 0) {
            // Auto-scale from trimmed dimensions
            b.scale = ref.trimmedHeight / b.trimmedHeight;
          }
          // else: keep manual `scale` from the BIKES literal as fallback
        });
        console.info('[hero] auto-scale computed against', ref.id,
          '— scales:', BIKES.map((b) => `${b.id}=${b.scale.toFixed(3)}`).join(', '));
      }
    } catch (err) {
      console.warn('[hero] auto-scale pass failed; using manual scales:', err);
    }
  }
  ...
  function selectBike(idx, fromUser) {
    ...
    tl.to(bikeWrap, { x: -60, opacity: 0, rotation: -8, duration: 0.3, ease: 'power2.in' })
      .add(() => {
        ...
        // NOTE: do NOT call updateLeaders here — the wrap is mid-transition
        // (x:-60, rotation:-8). getBoundingClientRect() would read those
        // displaced values and the lines would freeze at the offset
        // position even after the wrap returns to (0,0). Schedule it
        // for after the entrance fromTo completes (see onComplete below).
      })
      .fromTo(bikeWrap,
        { x: 120, opacity: 0, rotation: 30 },
        { x: 0, opacity: 1, rotation: 0, duration: 0.85, ease: 'power3.out',
          onComplete: () => {
            // Wrap is now back at its resting position — safe to compute
            // leader-line endpoints from its bounding rect.
  ...
  function runEntrance() {
    ...
    // (Brand lockup is no longer animated here — it lives in the persistent
    //  navbar and must be visible from page load. JS only fades the .bl-mark
    //  in once the white-grid knockout finishes; see prepareLogo above.)
  ...
  /* ============================================================
     LOGO KNOCKOUT
     ============================================================
     Strips the white-grid background from the supplied logo PNG so
     it sits cleanly on dark UI. Sets the result on:
       1. Every .bl-mark in the page (desktop navbar lockup + mobile-
          overlay lockup — both use the same class)
       2. Hero faint-S watermark (large faded mark, bottom-right of hero)
     ============================================================ */
  async function prepareLogo() {
    const marks = document.querySelectorAll('.bl-mark');
    // The new suzuki-s.png is a clean transparent-PNG of the S-mark, so
    // the white-grid knockout step is no longer required. Use it directly.
    const src = 'assets/images/logo/suzuki-s.png';
    ...
    // Hero watermark (large faint S behind the bike stage)
    const wm = document.getElementById('hero-watermark');
    if (wm) wm.style.backgroundImage = `url(${src})`;

    // Hide the type-fallback "S" once the real mark is ready
  ...
  window.addEventListener('load', async () => {
    /* Logo knockout runs on every page that has a .bl-mark in the navbar
       (hero.js is included on bikes.html, detail pages, etc. for this
       reason). Hero-specific behaviour below is gated on #hero. */
    if (document.querySelectorAll('.bl-mark').length > 0) {
      try {
        await withTimeout(prepareLogo(), 4000, 'logo prepare');
      } catch (err) {
        console.warn('[hero] prepareLogo threw:', err);
      }
    }
    ...
    if (!document.getElementById('hero')) {
      // Not a page with the hero — logo already handled, nothing more to do.
      return;
    }
    ...
    /* ----------------------------------------------------------
       Expose internals to the anchor-picker debug helper. Only
       happens when ?anchors is in the URL — otherwise this block
       does nothing and the global stays undefined.
       ---------------------------------------------------------- */
    if (ANCHOR_DEBUG) {
      window.__heroDebug = {
        BIKES,
      ...
      // Notify the debug script (deferred-loaded so it may already
      // be waiting for this signal).
      window.dispatchEvent(new CustomEvent('hero:debug-ready'));
      console.info('[hero] anchor-picker debug mode active. Open the panel top-right.');
    }
  });
})();
